...
class UserMenuIcon {
  /*
	...
	*/
  constructor(gameEngine, xCanvas, yCanvas, ctx, level) {
    Object.assign(this, {
      gameEngine,
      xCanvas,
      yCanvas,
...
class UndoIcon extends UserMenuIcon {
  /*
	...
	*/
  constructor(gameEngine, xCanvas, yCanvas, ctx, level) {
    super(gameEngine, xCanvas, yCanvas, ctx, level);
  ...
  /*
	...
	*/
  update() {
    if (this.level.newestTower !== null && !this.level.levelPaused) {
...
class SpeedIcon extends UserMenuIcon {
  ...
  drawSubLabel(ctx) {
    ...
    for (var i = 0; i < this.speedMultipliers.length; i++) {
      ...
      var xOffset =
        this.xCanvas + (this.iconBoxWidth / 3.5) * i + this.iconBoxWidth / 9;
      var verticalAlign = this.yCanvas + this.iconBoxHeight * 0.85;
      ...
      if (multiplier === this.sublabelStatus) {
        ...
        ctx.lineTo(
          xOffset + ctx.measureText(multiplier).width,
          verticalAlign + 3
        );
        ctx.lineWidth = 1;
        ctx.stroke();
