class FlyingEye extends Enemy {
    constructor(gameEngine, x, y, level, spawnTime) {
      ...
      // sprites
      this.attackImg = ASSET_MANAGER.getAsset("./sprites/monster/flyingeye/Attack.png");
      this.deathImg = ASSET_MANAGER.getAsset("./sprites/monster/flyingeye/Death.png");
      this.flyImg = ASSET_MANAGER.getAsset("./sprites/monster/flyingeye/Flight.png");
      this.takehitImg = ASSET_MANAGER.getAsset("./sprites/monster/flyingeye/Take Hit.png");
  
      // animations
      this.attackAnim = new Animator(this.attackImg, 0, 0, 150, 150, 8, 0.1, 0, false, true);
      this.deathAnim = new Animator(this.deathImg, 0, 0, 150, 150, 4, 0.2, 0, false, true);
      this.flyAnim = new Animator(this.flyImg, 0, 0, 150, 150, 8, 0.07, 0, false, true);
      this.takehitAnim = new Animator(this.takehitImg, 0, 0, 150, 150, 4, 0.15, 0, false, true);
      this.loadAnimation();
      ...
      for (var i = 0; i < this.gameEngine.entities.length; i++) {
        ...
        if (ent instanceof Tower) {
          if (canShoot(this, ent) && this.cooldownTime > this.fireRate) {
            this.cooldownTime = 0;
            this.state = 1;
            this.gameEngine.addEntity(new Bullet(this.gameEngine, this.x, this.y + this.yOffset, BULLETS["tomato"], ent, this));
          }
